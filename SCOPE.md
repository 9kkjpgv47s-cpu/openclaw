...
- When using `cursor-agent status` as the auth canary, apply the bounded **transient-only** retry described in `TOOLS.md` (security-status canary retry); never retry on explicit auth failure signals.
