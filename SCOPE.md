...
- When using `cursor-agent status` as a pre-flight auth canary, follow the **Security status canary (retry)** rules in `TOOLS.md` (bounded retries for transient errors only; never retry on clear auth/config failures).
