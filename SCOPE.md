...
- If ACP or `cursor-agent status` returns errors that look **transient** (e.g. **security status**, **canary**, HTTP **503**, “temporarily unavailable”), retry **up to 3 times** with short backoff, then report the outcome. Same idea as the runbook in `TOOLS.md` for host-side status checks.
